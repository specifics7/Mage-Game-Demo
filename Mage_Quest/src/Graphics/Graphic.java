...
public class Graphic extends JFrame{
	...
	private void init(){
		...
		// Find the display modes that are supported with the monitor
		ArrayList<String> displays = new ArrayList<>();
		for(int i = 0; i < dms.length; i++){
			int height = dms[i].getHeight();
			int width = dms[i].getWidth();
			int bits = dms[i].getBitDepth();
			displays.add(width + " x " + height + ", " + bits + " bits.");
		}
		// Export data to log file
		EZFileWrite ezw = new EZFileWrite("display_log.txt");
		for(String display:displays){
			ezw.writeLine(display);
		}
		ezw.saveFile();
		...
		int xres = 1920;
		int yres = 1080;
		int vindexN3 = isSupported(xres, yres, 32, dms);
		//int vindexN3 = isSupported(1920, 1080, 32, dms);
		//int vindexN3 = isSupported(1440, 900, 32, dms);
		...
		EZFileWrite ezw2 = new EZFileWrite("Scripts/logs/pf.txt"); // <- Relative to .classpath
		ezw2.writeLine(graphLog);
		ezw2.saveFile();
	}
